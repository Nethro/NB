...
mobile: MethodMobile, 
browser: opcion == '1' ? ['FlashBot', 'Edge', '2.0.0'] : methodCodeQR ? ['FlashBot', 'Edge', '2.0.0'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "FlashBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const subBotDirectory = './FlashBot';
...
(async () => {
global.conns = [];
const mainBotAuthFile = 'FlashSession';
try {
...
try {
const listaDirectorios = readdirSync('./FlashBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./FlashBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./FlashBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./FlashBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./FlashSession/', './FlashBot/']
